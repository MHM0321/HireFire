...
export default function MyPerformancePg() {
  ...
  const ratingsData = [
    { rating: 5, review: 'Excellent work and very professional.', reviewer: 'John Doe' },
    { rating: 4, review: 'Good service but could be faster.', reviewer: 'Jane Smith' },
          ...
          {historyData.map((item, index) => (
            <View key={index} style={styles.listItem}>
              <View style={styles.listItemLeft}>
                <FontAwesome5 name="briefcase" size={18} color="#666666" style={{ marginRight: 8 }} />
...
<View style={styles.section}>
  ...
  <View style={styles.analyticsSummary}>
    <Text style={styles.analyticsText}>Total Works: {analyticsData.total}</Text>
    <Text style={styles.analyticsText}>Completed: {analyticsData.completed}</Text>
    <Text style={styles.analyticsText}>Not Completed: {analyticsData.notCompleted}</Text>
  </View>

  {/* Calculate percentages */}
  {(() => {
    const total = analyticsData.completed + analyticsData.notCompleted;
    const completedPercent = Math.round((analyticsData.completed / total) * 100);
    const notCompletedPercent = Math.round((analyticsData.notCompleted / total) * 100);
